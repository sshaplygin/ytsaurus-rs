...
import yt.wrapper.schema as typed_schema  # ty: ignore[unresolved-import]

# The bindings resolve `yt.yson.yson_types` lazily from inside C++. Importing it
# here is load-bearing, not tidiness.
import yt.yson.yson_types  # noqa: F401  # ty: ignore[unresolved-import]
from yt.wrapper.format import StructuredSkiffFormat  # ty: ignore[unresolved-import]
from yt.wrapper.schema import yt_dataclass  # ty: ignore[unresolved-import]
from yt_yson_bindings import SkiffSchema, dump_skiff, load_skiff  # ty: ignore[unresolved-import]
...
STRUCTURED_VECTORS: list[StructuredVector] = [
    {
        ...
        # A checker that does not run class decorators sees a class with no
        # constructor arguments and reports every keyword here; `ty` resolves
        # the decorator and needs nothing. If the checker is ever changed again,
        # this is the construct to look at first.
        "rows": [
            Composite(
                id=1,
                tags=[],
                nested=Inner(x=7, y="z"),
                maybe=None,
            ),
            Composite(
                id=2,
