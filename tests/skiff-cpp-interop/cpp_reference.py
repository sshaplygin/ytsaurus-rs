...
def optional(wire_type: str, name: str) -> dict:
...
def skiff_schema(schema: dict) -> SkiffSchema:
...
def encode_record_row(schema: dict, fields: dict, table_index: int) -> bytes:
...
def verify_record(vector: dict, encoded: bytes) -> None:
...
STRUCTURED_VECTORS = [
    {
        ...
        "dataclass": Composite,
        "rows": [
            Composite(id=1, tags=[], nested=Inner(x=7, y="z"), maybe=None),
            Composite(id=2, tags=["ab", ""], nested=Inner(x=-1, y=""), maybe=-5),
        ],
    },
]


def structured_format(dataclass, for_reading: bool) -> StructuredSkiffFormat:
...
def inferred_schema(vector: dict):
    ...
    """
    return typed_schema._row_py_schema_to_skiff_schema(
...
def describe(schema, indent: int = 0) -> list[str]:
...
def main() -> int:
    ...
    for vector in STRUCTURED_VECTORS:
        encoded = encode_structured(vector)
        verify_structured(vector, encoded)
        doc = vector["doc"] + ["", "Schema derived by the typed layer:"]
        doc += ["  " + line for line in describe(inferred_schema(vector))]
        ...
        block = [
            (f"{len(vector['rows'])} rows, {len(encoded)} bytes", encoded),
        ]
        (HERE / vector["file"]).write_text(render(doc, block, width=16))
        print(f"{vector['file']}: {len(vector['rows'])} rows, {len(encoded)} bytes")
